...
from . import check_domain, OUTFILE_RAW, key_extensions
from .... import CONFIG, Config

print(Config._mastodon)

DATAREPO = CONFIG[Config._mastodon][Config._datarepo]
EXTS = CONFIG[Config._mastodon][Config._rdd][key_extensions]
...
parser.add_argument('--extensions', nargs='+', default=EXTS, help='The extensions that should be appended to the input words to test whether the server at the generated domain is running Mastodon.')
...
parser.add_argument('--substitutions', help='Common substitutions of numbers for letters; generates all possibilities.', default=False, action='store_true')
...
with open(args.words, 'r') as f:
    ...
    if args.substitutions:
        subst_words = set(words)
        for word in words:
            subst_words.update(word.replace(let, num) for (let, num) in substitutions)
        words = list(subst_words)
