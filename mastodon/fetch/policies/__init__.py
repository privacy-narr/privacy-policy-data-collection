...
def extract_policy(url):
    ...
    try:
        ...
        with open(f'pp_fetch_failures_{today.year}_{today.month:02d}_{today.day:02d}.err', 'a') as f:
            f.write(url + '\n')
...
def get_servernames(filepath, format):
    with open(filepath, 'r') as f:
        if format == 'jsons':
            return [json.loads(obj)['domain'] for obj in f.readlines()]
            ...
            if headersp:
                ...
                url_index = 1
            return [row[url_index] for row in reader]
        elif format == 'txt':
            return f.readlines()
        else:
